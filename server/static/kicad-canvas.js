...
(() => {
  ...
  var PCB_SHEET = "#C872AB";
  var PCB_PAGE_LIMITS = "#848484";
  ...
  var TEXT_INTERLINE = 1.68 * 0.9583;
  function lineOffsets(n, vjust) {
  ...
  function buildSheetGeom(item, n) {
    ...
    if (inst) for (const proj of kids(inst, "project")) for (const path of kids(proj, "path")) {
      const pg = kid(path, "page");
      if (pg) {
        item.page = str(pg[1]);
        break;
      }
  ...
  function newDoc(type) {
    return {
      ...
      lib: /* @__PURE__ */ new Map(),
      page: PAPER_SIZES.A4.slice(),
  ...
  var MM_PER_MIL = 25.4 / 1e3;
  var mm2mils = (mm) => Math.round(mm * 1e3 / 25.4);
  var milsPage = (w, h) => [w * MM_PER_MIL, h * MM_PER_MIL];
  var mmPage = (w, h) => milsPage(mm2mils(w), mm2mils(h));
  var userMm = (mm) => Math.max(mm, 10 * MM_PER_MIL);
  var PAPER_SIZES = {
    A5: mmPage(210, 148),
    A4: mmPage(297, 210),
    A3: mmPage(420, 297),
    ...
    A1: mmPage(841, 594),
    A0: mmPage(1189, 841),
    A: milsPage(11e3, 8500),
    B: milsPage(17e3, 11e3),
    C: milsPage(22e3, 17e3),
    D: milsPage(34e3, 22e3),
    E: milsPage(44e3, 34e3),
    GERBER: milsPage(32e3, 32e3),
    USLetter: milsPage(11e3, 8500),
    USLegal: milsPage(14e3, 8500),
    USLedger: milsPage(17e3, 11e3)
  };
  ...
  function parseDoc(text, docType) {
    ...
    for (let j = 1; j < tree.length; j++) {
      ...
      const k = node[0];
      if (k === "paper") {
        doc.page = paperSize(node);
        doc.paper = str(node[1]) || "A4";
        doc.portrait = doc.page[1] > doc.page[0];
      } else if (k === "title_block") {
        doc.titleBlock = titleBlockOf(node);
        continue;
      } else if (k === "sheet_instances") {
        const p = kid(node, "path");
        const pg = p && kid(p, "page");
        if (pg) doc.pageNumber = str(pg[1]) || "1";
        continue;
      } else if (k === "version") {
  ...
  // web/canvas/sheet.ts
  var DEFAULT_DRAWING_SHEET = `(kicad_wks (version 20210606) (generator pl_editor)
...
(line (name "") (start 26 8.5) (end 26 2))
  ...
  var WKS_DEFAULTS = { textSizeX: 1.5, textSizeY: 1.5, lineWidth: 0.15, textLineWidth: 0.15, left: 10, right: 10, top: 10, bottom: 10 };
  var DS_MAX_REPEAT_COUNT = 100;
  var CORNERS = { ltcorner: "lt", lbcorner: "lb", rbcorner: "rb", rtcorner: "rt" };
  function coordOf(node, from) {
    const c = { x: num(node[from], 0), y: num(node[from + 1], 0), anchor: "rb" };
    for (let j = from + 2; j < node.length; j++) {
      const a = CORNERS[str(node[j])];
      if (a) c.anchor = a;
    }
    return c;
  }
  function parseDrawingSheet(text) {
    ...
    for (let j = 1; j < tree.length; j++) {
      ...
      if (node[0] === "setup") {
        for (const s of node.slice(1)) {
          ...
          if (s[0] === "textsize") {
            setup.textSizeX = num(s[1], setup.textSizeX);
            setup.textSizeY = num(s[2], setup.textSizeY);
      ...
      const it = {
        ...
        maxlen: 0,
        maxheight: 0,
        text: kind === "tbtext" ? str(node[1]) : ""
      };
      for (const c of node.slice(1)) {
        if (!Array.isArray(c)) continue;
        switch (c[0]) {
          case "start":
            it.start = coordOf(c, 1);
            break;
          case "end":
            it.end = coordOf(c, 1);
            break;
          case "pos":
            ...
            break;
          case "repeat":
            it.repeat = Math.min(Math.max(Math.round(num(c[1], 1)), 1), DS_MAX_REPEAT_COUNT);
            break;
          case "incrx":
            it.incr[0] = num(c[1], 0);
            break;
          case "incry":
            ...
            break;
          case "incrlabel":
            it.incrLabel = Math.round(num(c[1], 1));
            break;
          case "linewidth":
            it.lineWidth = num(c[1], 0);
            break;
          case "maxlen":
            it.maxlen = num(c[1], 0);
            break;
          case "maxheight":
            it.maxheight = num(c[1], 0);
            break;
          case "rotate":
            it.rot = num(c[1], 0);
            break;
            ...
            for (const f of c.slice(1)) {
              if (f === "bold") it.bold = true;
                ...
                if (f[0] === "size") {
                  ...
                  const a = num(f[4], 1);
                  if (a > 0) it.color = `rgba(${num(f[1])},${num(f[2])},${num(f[3])},${+a.toFixed(3)})`;
                }
              }
            }
            break;
  ...
  var DEFAULT_SHEET = null;
  function defaultDrawingSheet() {
    if (!DEFAULT_SHEET) DEFAULT_SHEET = parseDrawingSheet(DEFAULT_DRAWING_SHEET);
    return DEFAULT_SHEET;
  }
  var ALPHABET = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  function indexFromAlphabetic(s, alphabet) {
  ...
  function alphabeticFromIndex(n, alphabet) {
    const alpha = alphabet || ALPHABET, radix = alpha.length;
    let out = "", first = true;
    do {
      let mod = n % radix;
  ...
  function incrementString(s, delta) {
    if (!s) return null;
    let remaining = s;
    const parts = [];
    let good = 0;
    while (good <= 0 && remaining) {
      ...
      if (m) {
        parts.push({ s: m[0], type: "int" });
        good++;
      } else if (m = /([a-z]+|[A-Z]+)$/.exec(remaining)) {
        parts.push({ s: m[0], type: "alpha" });
        good++;
  ...
  function incrementLabel(base, delta) {
    const s = incrementString(base, delta);
    return s === null ? base : s;
  }
  function expandTextVars(text, resolve, depth) {
    const src = str(text);
  ...
  var TWO = (n) => (n < 10 ? "0" : "") + n;
  var joinPath = (dir, name) => dir ? dir.replace(/\/+$/, "") + "/" + name.replace(/^\/+/, "") : name;
  var PRODUCT_NAME = "KiCad E.D.A.";
  var WS_TOKENS = /* @__PURE__ */ new Set([
    "KICAD_VERSION",
    "#",
    "##",
    "SHEETNAME",
  ...
  var TB_TOKENS = /* @__PURE__ */ new Set([
    "ISSUE_DATE",
    "CURRENT_DATE",
  ...
  function varResolver(vars, opts) {
    ...
    const fromProject = (token) => {
      const v = project ? project[token] : void 0;
    ...
    return (token) => {
      ...
      if (withTb && TB_TOKENS.has(token)) {
        ...
        if (raw !== void 0 && raw !== null) {
          ...
          if (value === "${" + token + "}") {
            const p = fromProject(value);
            return p === null ? { ref: value } : p;
          }
  ...
  function sheetTextPen(pen, size, bold) {
    let t = bold ? 0 : pen;
    if (t <= 0) t = bold ? size / 5 : size / 8;
    return Math.min(t, size * 0.25);
  }
  function buildDrawingSheet(page, vars, wks, firstPage, project) {
    ...
    for (const it of sheet.items) {
      ...
      if (!isText) {
        bx0 = Math.min(bx0, e0[0]);
        by0 = Math.min(by0, e0[1]);
        bx1 = Math.max(bx1, e0[0]);
        by1 = Math.max(by1, e0[1]);
      }
      ...
      if (isText) {
        ...
        if (it.maxlen > 0) {
          const w = textWidth(full, sizeX, pen);
          if (w > it.maxlen) size *= it.maxlen / w;
        }
  ...
  var SHEET_CACHE = /* @__PURE__ */ new WeakMap();
  function documentDrawingSheet(doc) {
    ...
    const key = [
      ...
      doc.projectName,
      doc.kicadVersion
    ].join("\0") + "\0" + JSON.stringify(doc.titleBlock || null) + "\0" + JSON.stringify(doc.sheetVars || null) + "\0" + JSON.stringify(doc.projectVars || null);
  ...
  function drawText(ctx, g, s, docType, minW, colorOverride, base, flip, bgColor) {
    ...
    if (!font) {
      font = `${g.italic ? "italic " : ""}${g.bold ? "600 " : ""}${g.size * FONT_EM}px ${FONT_FAMILY}`;
      if (FONT_CACHE.size < 512) FONT_CACHE.set(key, font);
    }
    ctx.font = font;
    ctx.textAlign = h;
    ctx.textBaseline = "alphabetic";
    ...
    if (lines) {
      ...
      for (let i = 0; i < lines.length; i++) {
        const y = base0 + offs[i] * step;
        if (boldPass) ctx.strokeText(lines[i], 0, y);
  ...
  function drawDrawingSheet(ctx, doc, s, minW) {
    const isPcb = doc.type === "pcb";
    ...
    for (const it of items) {
      ...
      drawText(ctx, {
        ...
        minPx: 3
      }, s, doc.type, minW, null, null, false, null);
    }
  ...
  function svgGeom(g, hairline, isPcb, bg) {
    ...
    if (t === "text") {
      ...
      if (g.multiline && g.text.indexOf("\n") >= 0) {
        ...
        body = ls.map((t2, i) => `<text y="${fmt(base0 + offs[i] * step)}" ${attrs}>${esc(t2)}</text>`).join("");
  ...
  function svgDrawingSheet(doc, hairline) {
    const isPcb = doc.type === "pcb";
    const ink = isPcb ? PCB_SHEET : SCH.frame;
    const out = [];
    for (const it of documentDrawingSheet(doc)) {
      ...
      else if (it.t === "text") out.push(svgGeom({
        t: "text",
        x: it.x,
        y: it.y,
        text: it.text,
