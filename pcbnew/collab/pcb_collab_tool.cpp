...
void PCB_COLLAB_TOOL::OnSnapshotRequest( const wxString& aDocId )
{
    // pcbnew holds a single board doc, so the requested id is always its own.
    if( m_sync )
...
void PCB_COLLAB_TOOL::OnJoinRefused( const wxString& aDocId, const wxString& aCode )
{
    ...
    editFrame->ShowInfoBarError( aCode == wxS( "not_found" )
            ? _( "This board is no longer part of the shared project; your edits to it are "
                 "not being shared." )
...
void PCB_COLLAB_TOOL::updateCommentCard()
{
    ...
    if( !overCard && editFrame->GetCanvas()->GetScreenRect().Contains( mouse ) )
    {
    ...
    {
        m_pinHover = root >= 0;
        editFrame->GetCanvas()->SetCurrentCursor( m_pinHover ? KICURSOR::HAND : KICURSOR::ARROW );
    }
