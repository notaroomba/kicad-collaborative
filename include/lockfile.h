...
public:
            ...
            m_removeOnRelease( aRemoveOnRelease )
    {
        ...
        if( !CreateLocks() )
        {
            // Look, don't touch: honor a live lock (e.g. a stock KiCad's) but
            // never write one of our own.
            bool heldByAnother = false;

            m_lockFilename = LockPathFor( filename );
            m_removeOnRelease = false;

            if( m_lock.OpenForInspect( m_lockFilename, heldByAnother ) )
                readOwner();

            m_status = !heldByAnother;
            return;
        }
